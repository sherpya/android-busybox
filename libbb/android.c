 ...
 * Copyright (C) 2012-2020 by Gianluigi Tiesi <sherpya@netfarm.it>
 *
...
// bionic
char *_hasmntopt(const struct mntent* mnt, const char* opt);
char *_hasmntopt(const struct mntent* mnt, const char* opt) {
...
weak_alias(_hasmntopt, hasmntopt);
#endif /* __ANDROID_API__ < 26 */

#if __ANDROID_API__ < 28
/* from gnulib - GNU GPLv2 */
int _getlogin_r(char *name, size_t size);
int _getlogin_r(char *name, size_t size)
{
    char *n;
    size_t nlen;
...
weak_alias(_getlogin_r, getlogin_r);
